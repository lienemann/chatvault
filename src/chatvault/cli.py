...
contact_app = typer.Typer(
    name="contact", help="Address-book entries and pins.", no_args_is_help=True
)
chat_app = typer.Typer(
    name="chat", help="Browse and operate on chats (groups + 1:1).", no_args_is_help=True
)
...
DEFAULT_MEDIA_ROOT = Path("/storage/emulated/0/Android/media/com.whatsapp/WhatsApp/Media")
...
def extract(
    backup: Annotated[
        ...
        typer.Option(
            "--backup",
            help="Encrypted msgstore.db.crypt15. If omitted, uses staging/msgstore.db if present.",
        ),
    ] = None,
    status_backup: Annotated[
        ...
        typer.Option(
            "--status-backup",
            help="Encrypted status_backup.db.crypt15. Defaults to a sibling of --backup.",
        ),
    ] = None,
    ...
    keep_decrypted: Annotated[
        bool,
        typer.Option(
            "--keep-decrypted", help="Don't delete the staging plaintext DBs after extract"
        ),
    ] = False,
...
def key_set(
    value: Annotated[
        str | None, typer.Argument(help="64-char hex key. If omitted, read from stdin.")
    ] = None,
) -> None:
...
def contacts_sync(
    default_country: Annotated[
        ...
        typer.Option(
            "--default-country",
            help="E.164 country code prefix without '+', for unprefixed numbers.",
        ),
    ] = "41",
    from_stdin: Annotated[
        bool,
        typer.Option(
            "--stdin", help="Read termux-contact-list-style JSON from stdin instead of running it."
        ),
    ] = False,
) -> None:
...
def contacts_pin(
    phone: Annotated[
        str,
        typer.Argument(
            help="Phone number or phone-JID (e.g. '+49 172 3105522' or '491723105522@s.whatsapp.net')."
        ),
    ],
    name: Annotated[str, typer.Argument(help="Display name to use for this number.")],
    default_country: Annotated[str, typer.Option("--default-country")] = "41",
) -> None:
...
def contact_list(
    source: Annotated[
        str | None, typer.Option("--source", help="Filter: address_book / vcard / manual.")
    ] = None,
...
def contact_unresolved(
    min_messages: Annotated[
        int, typer.Option("--min-messages", help="Only show senders with at least N messages.")
    ] = 1,
    ...
    csv_out: Annotated[
        ...
        typer.Option(
            "--csv",
            help="Write a phone,name CSV starter file (importable via `contact import-csv`).",
        ),
    ] = None,
    fmt: Annotated[str, typer.Option("--format", "-f", help="'table' or 'json'.")] = "table",
) -> None:
    ...
    if csv_out:
        ...
        console.print(
            f"[green]✓[/] {csv_out} ({len(rows)} rows). Edit, then `chatvault contact import-csv {csv_out}`."
        )
        return
...
def contacts_import_vcard(
    ...
    try:
        ...
        conn.close()
    console.print(f"[green]✓[/] {result.total} numbers seen, +{result.set_count} new/updated")
...
def chat_list(
    kind: Annotated[
        str | None, typer.Option("--kind", help="Filter by kind: group/user/lid/newsletter/...")
    ] = None,
    since: Annotated[
        str | None,
        typer.Option("--since", help="Only chats with activity at/after this ISO date."),
    ] = None,
    until: Annotated[
        str | None,
        typer.Option("--until", help="Only chats with activity at/before this ISO date."),
    ] = None,
...
def chat_alias(
    ...
    chat: Annotated[
        str | None,
        typer.Argument(
            help="Chat reference (group subject, contact name, phone, JID, or # index)."
        ),
    ] = None,
    remove: Annotated[
    ...
    Examples:
        chatvault chat alias mom "Sam Roe"
        chatvault chat alias work "Finance"
...
def chat_members(
    chat: Annotated[
        str, typer.Argument(help="Chat identifier (group subject, contact name, phone, or JID).")
    ],
    ...
    _check_format(fmt)
    from .queries.chats import members_for
...
def chat_export(
    ...
    include_media: Annotated[
        bool,
        typer.Option(
            "--include-media/--no-include-media", help="Hardlink media files into <out>/media/."
        ),
    ] = True,
) -> None:
    ...
    try:
        chat_jid = _resolve_chat_or_exit(conn, chat)
        info = conn.execute("SELECT subject, kind FROM chats WHERE jid = ?", (chat_jid,)).fetchone()
        title = info["subject"] or (
            NameResolver(conn).resolve(chat_jid) if info["kind"] in ("user", "lid") else chat_jid
        )
    ...
    if include_media and media_rows:
        ...
        for r in media_rows:
            ...
            if r["mirrored_path"] and Path(r["mirrored_path"]).exists():
                ...
                src = (
                    cand_mirror
                    if cand_mirror.exists()
                    else (cand_live if cand_live.exists() else None)
                )
            if not src:
                media_missing += 1
                continue
...
def chat_info(
    ...
    from .queries.chats import chat_info as _info
...
def timeline(
    ...
    for r in rows:
        ...
        if not body and not media:
            body = "<no body>"
        console.print(
            f"[dim]{r['ts'][:19]}[/] [bold]{r['subject'] or r['chat_jid']}[/]: {body}{media}"
        )
...
def forgotten(
    days: Annotated[
        int, typer.Option("--days", help="Threshold in days since last outgoing.")
    ] = 365,
    limit: Annotated[int, typer.Option("--limit", "-n")] = 50,
    fmt: Annotated[str, typer.Option("--format", "-f", help="'table' or 'json'.")] = "table",
) -> None:
...
def search(
    ...
    since: Annotated[str | None, typer.Option("--since", help="ISO date or datetime.")] = None,
    until: Annotated[str | None, typer.Option("--until")] = None,
    limit: Annotated[int, typer.Option("--limit", "-n")] = 50,
    fmt: Annotated[str, typer.Option("--format", "-f", help="'table' or 'json'.")] = "table",
) -> None:
...
def status_list(
    kind: Annotated[str | None, typer.Option("--kind", help="'own' or 'received'.")] = None,
    sender: Annotated[
        str | None, typer.Option("--sender", help="Substring of resolved sender name.")
    ] = None,
    since: Annotated[str | None, typer.Option("--since")] = None,
    until: Annotated[str | None, typer.Option("--until")] = None,
    limit: Annotated[int, typer.Option("--limit", "-n")] = 50,
...
def status_media(
    status_id: Annotated[
        str,
        typer.Argument(
            help="Status id, or shortcut: 'latest', 'top', or index. Defaults to 'latest'."
        ),
    ] = "latest",
    ...
    if sa["file_path"]:
        ...
        candidates.append(
            (
                "live source",
                DEFAULT_MEDIA_ROOT / rel[len("Media/") :]
                if rel.startswith("Media/")
                else DEFAULT_MEDIA_ROOT.parent / rel,
            )
        )
        console.print("\n[bold]Local paths[/]")
...
def status_summary(
    ...
    limit: Annotated[int, typer.Option("--limit", "-n")] = 50,
    sort: Annotated[
        str, typer.Option("--sort", help="'ts' (newest first) or 'views' (most-viewed first).")
    ] = "ts",
    fmt: Annotated[str, typer.Option("--format", "-f", help="'table' or 'json'.")] = "table",
) -> None:
...
def receipts(
    ...
    for r in rows:
        console.print(f"[dim]{r['ts'][:19]}[/] [cyan]{r['recipient']}[/] {r['status']}")
...
def stats(
    ...
    try:
        ...
        conn.close()
    if fmt == "json":
        _emit_json([{"label": label, "count": c} for label, c in rows])
        return
...
def mirror_start(
    media_root: Annotated[Path, typer.Option("--media-root")] = Path(
...
# ---------------------------------------------------------------------------
# transcribe — local whisper.cpp transcription of snapshotted audio messages
# ---------------------------------------------------------------------------
...
def transcribe(
    model: Annotated[
        ...
        typer.Option(
            "--model",
            help="Path to a whisper.cpp ggml model. Defaults to "
            "$CHATVAULT_WHISPER_MODEL or ~/.local/share/whisper/ggml-small.bin.",
        ),
    ] = None,
    language: Annotated[
        str,
        typer.Option(
            "--language", "-l", help="ISO 639-1 code or 'auto' for whisper's auto-detect."
        ),
    ] = "auto",
    chat: Annotated[
        str | None,
        typer.Option(
            "--chat", help="Restrict to a single chat (group subject, name, phone, JID, or alias)."
        ),
    ] = None,
    limit: Annotated[
        int | None,
        typer.Option("--limit", "-n", help="Stop after N transcriptions (newest first)."),
    ] = None,
    threads: Annotated[
        int | None,
        typer.Option("--threads", "-t", help="Threads passed to whisper-cli (-t)."),
    ] = None,
    batch_size: Annotated[
        int,
        typer.Option(
            "--batch-size",
            help="How many voice notes share one whisper-cli invocation (model loads once per batch).",
        ),
    ] = 16,
    force: Annotated[
        bool,
        typer.Option(
            "--force", help="Re-transcribe audio messages that already have a transcript."
        ),
    ] = False,
    dry_run: Annotated[
        bool,
        typer.Option("--dry-run", help="List candidates without invoking ffmpeg/whisper-cli."),
    ] = False,
    backend: Annotated[
        str,
        typer.Option(
            "--backend",
            help="'cli' (whisper-cli on PATH) or 'pywhispercpp' (in-process via `pip install pywhispercpp`).",
        ),
    ] = "cli",
    media_root: Annotated[
        Path,
        typer.Option(
            "--media-root",
            help="Fallback source-app Media root, used when the mirror copy is absent.",
        ),
    ] = DEFAULT_MEDIA_ROOT,
) -> None:
    """Transcribe snapshotted audio messages with local whisper.cpp.

    Two backends are available:

      • ``cli`` (default) — shells out to ``whisper-cli`` from whisper.cpp.
        Requires ``ffmpeg`` and ``whisper-cli`` on PATH.
      • ``pywhispercpp`` — in-process via the `pywhispercpp` pip package. Still
        needs ``ffmpeg`` for decoding ``.opus`` → 16 kHz WAV, but skips the
        external CLI install.

    Note: at the time of writing, none of the pip-installable whisper stacks
    route inference through the Hexagon NPU or Adreno GPU on Termux — all run
    on the ARM CPU. Use ``--threads`` to throttle / saturate as needed.

    Re-runs are idempotent: messages with an existing ``whisper_transcriptions``
    row are skipped unless ``--force`` is set. The text is later joined into
    ``chatvault chat digest`` output.
    """
    from .whisper import (
        default_model_path,
        detect_tools,
        iter_audio_candidates,
        resolve_audio_path,
        transcribe_all,
    )

    if backend not in ("cli", "pywhispercpp"):
        err_console.print(f"[red]--backend must be 'cli' or 'pywhispercpp' (got {backend!r}).[/]")
        raise typer.Exit(code=2)

    paths = Paths.default()
    paths.ensure()
    tools = detect_tools()
    if not tools.ok(backend) and not dry_run:
        hint = (
            "`pkg install ffmpeg && pkg install whisper.cpp`"
            if backend == "cli"
            else "`pkg install ffmpeg && pip install pywhispercpp`"
        )
        err_console.print(
            f"[red]missing tools for backend={backend!r}: {', '.join(tools.missing(backend))}[/]\n"
            f"[dim]Install: {hint}[/]"
        )
        raise typer.Exit(code=2)

    model_path = (model or default_model_path(paths)).expanduser()
    if not dry_run and not model_path.exists():
        err_console.print(
            f"[red]whisper model not found:[/] {model_path}\n"
            "[dim]Download a ggml model (e.g. base) and either pass --model PATH "
            "or set $CHATVAULT_WHISPER_MODEL.[/]"
        )
        raise typer.Exit(code=2)
    ...
    try:
        ...
        if dry_run:
            live_root = media_root if media_root.exists() else None
            shown = 0
            for row in iter_audio_candidates(conn, chat_jid=chat_jid, force=force, limit=limit):
                src = resolve_audio_path(row, paths, live_root)
                mark = "[green]✓[/]" if src else "[red]✗[/]"
                console.print(f"  {mark} {row.message_id} [dim]{src or '(no file)'}[/]")
                shown += 1
