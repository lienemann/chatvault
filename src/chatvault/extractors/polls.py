    ...
    with transaction(archive):
        for row in source.execute(POLLS_QUERY):
            ...
            archive.execute(
                ...
                (
                    message_id,
                    row["selectable_count"],
                    row["poll_type"],
                    row["content_type"],
                    ms_to_iso(row["end_time_ms"]),
                    row["allow_add_option"],
                    row["hide_participants"],
                    row["invalid"],
                ),
            )
            res.rows_written += 1
        ...
        for _rowid, message_id in rowid_to_msg.items():
