...
from chatvault.db import get_state_int, set_state_int, transaction
...
def _build_aux(source: sqlite3.Connection, since_rowid: int) -> dict[str, Any]:
    ...
    forwarded: dict[int, dict[str, Any]] = {}
    ...
    with transaction(archive):
        for row in source.execute(MESSAGE_QUERY, (since_rowid,)):
            ...
            if (
                ...
                and not from_me
                and chat_jid
                and chat_jid.endswith("@s.whatsapp.net")
            ):
                sender_jid = chat_jid
            ...
            if (q := aux["quoted"].get(rowid)) is not None:
                archive.execute(
                    ...
                    (
                        ...
                        q["quoted_key_id"],
                        q["quoted_text"],
                        q["quoted_message_type"],
                        q["quoted_sender_jid"],
                    ),
                )

            if (lp := aux["links"].get(rowid)) is not None:
                archive.execute(
                    "INSERT OR REPLACE INTO message_link_previews(message_id, url, title, description) "
                    "VALUES(?, ?, ?, ?)",
                    (message_id, lp["url"], lp["title"], lp["description"]),
                )
            ...
            if (al := aux["albums"].get(rowid)) is not None:
                archive.execute(
                    ...
                    (
                        ...
                        al["image_count"],
                        al["video_count"],
                        al["expected_image_count"],
                        al["expected_video_count"],
                    ),
                )
