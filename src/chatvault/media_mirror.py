...
def _link_or_copy(src: Path, dst: Path) -> str:
    """Materialise dst from src. Returns 'link', 'copy', or 'noop' (already there)."""
...
def snapshot_pass(
    ...
    for sub in SUBDIRS:
        ...
        for path in sub_root.rglob("*"):
            ...
            try:
                mode = _link_or_copy(path, dst)
                if mode == "noop":
                    continue
                _record(conn, path, dst, is_status=is_status, is_view_once=False)
                size = path.stat().st_size
                _log_mirrored(mode, size, path.relative_to(media_root))
                res.new_files += 1
                res.bytes += size
    ...
    try:
        ...
        for raw_line in proc.stdout:
            ...
            try:
                ...
                _record(conn, path, dst, is_status=is_status, is_view_once=False)
                conn.commit()
                _log_mirrored(mode, path.stat().st_size, Path(*rel_parts))
            except OSError as exc:
                log.warning("mirror failed for %s: %s", path, exc)
    finally:
        proc.terminate()
