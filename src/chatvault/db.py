...
def get_state(conn: sqlite3.Connection, key: str, default: str | None = None) -> str | None:
    row = conn.execute("SELECT value FROM extraction_state WHERE key = ?", (key,)).fetchone()
...
def insert_or_ignore(conn: sqlite3.Connection, table: str, row: dict[str, Any]) -> None:
