...
from chatvault.identities import NameResolver, jid_kind
...
def forgotten_contacts(
    ...
    LIMIT ?
    """
    return [dict(r) for r in conn.execute(sql, (f"-{days}", limit))]


def chat_member_explain(conn: sqlite3.Connection, chat_jid: str, member_jid: str) -> dict[str, Any]:
