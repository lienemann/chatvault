...
from chatvault.extractors import ms_to_iso
...
def receipts_for(
    ...
    row = archive.execute("SELECT key_id FROM messages WHERE id = ?", (message_id,)).fetchone()
    if not row:
    ...
    try:
        for r in src.execute(QUERY, (key_id,)):
            ...
            out.append(
                {
                    "ts": (
                        ms_to_iso(r["read_ts_ms"])
                        or ms_to_iso(r["played_ts_ms"])
                        or ms_to_iso(r["ts_ms"])
                        or ""
                    ),
