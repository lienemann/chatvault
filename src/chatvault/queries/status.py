...
def resolve_own_status_id(conn: sqlite3.Connection, selector: str) -> str | None:
    ...
    if sel == "latest":
        row = conn.execute(
            "SELECT id FROM status_archive WHERE kind='own' ORDER BY ts DESC NULLS LAST LIMIT 1"
        ).fetchone()
...
def own_status_view_summary(
    ...
    *,
    ...
    if until:
        where += " AND s.ts <= ?"
        params.append(until)
    order = "view_count DESC, s.ts DESC" if sort == "views" else "s.ts DESC NULLS LAST"
    sql = (
