...
def chat_message_at(conn: sqlite3.Connection, chat_jid: str, idx: int) -> str:
...
def resolve_message(conn: sqlite3.Connection, query: str) -> str:
    ...
    """
    parts = query.split(":")
    if len(parts) >= 3 and "@" in parts[0] and parts[1] in ("0", "1", "?") and parts[-1]:
...
def list_chats(
    ...
    *,
    ...
    if kind:
        ...
        params.append(kind)
    if since:
        where.append("(SELECT MAX(ts) FROM messages m WHERE m.chat_jid = c.jid) >= ?")
        params.append(since)
    if until:
        where.append("(SELECT MAX(ts) FROM messages m WHERE m.chat_jid = c.jid) <= ?")
        params.append(until)
...
def resolve_chat(conn: sqlite3.Connection, query: str) -> str:
    ...
    from chatvault.contacts import normalize_number
    ...
    # phone-number path so things like `41790001122` work.
    if sel.isdigit() and len(sel) <= CHAT_LIST_CACHE_INDEX_MAX_DIGITS:
        idx = int(sel)
        cached = _load_chat_list_cache(paths.cache_dir)
        if cached is None:
            msg = f"chat #{idx}: no `chat list` cache yet. Run `chatvault chat list` first."
            raise LookupError(msg)
    ...
    contact_rows = list(
        conn.execute(
            "SELECT phone_jid, name FROM contacts WHERE LOWER(name) LIKE ? ORDER BY name",
            (f"%{query.lower()}%",),
        )
    )
    if contact_rows:
        ...
        if not active:
            ...
            return str(contact_rows[0]["phone_jid"])
        if len(contact_rows) > 1 and len({r["pname"] for r in active}) > 1:
            names = "\n  ".join(f"{cr['name']}  [{cr['phone_jid']}]" for cr in contact_rows[:10])
            msg = f"multiple contacts match {query!r}:\n  {names}"
            raise LookupError(msg)
        return str(active[0]["jid"])
...
def members_for(
    ...
    chat_kind_row = conn.execute(
        "SELECT kind FROM chats WHERE jid = ?",
        (chat_jid,),
    ).fetchone()
...
def chat_info(conn: sqlite3.Connection, chat_jid: str) -> dict[str, Any]:
    ...
    info["first_message_ts"] = conn.execute(
        "SELECT MIN(ts) FROM messages WHERE chat_jid = ?",
        (chat_jid,),
    ).fetchone()[0]
    info["last_message_ts"] = conn.execute(
        "SELECT MAX(ts) FROM messages WHERE chat_jid = ?",
        (chat_jid,),
    ).fetchone()[0]
    info["reaction_count"] = conn.execute(
        "SELECT COUNT(*) FROM reactions WHERE parent_chat_jid = ?",
        (chat_jid,),
    ).fetchone()[0]
    return info
