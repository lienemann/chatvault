...
def restore_manual_pins(conn: sqlite3.Connection, config_dir: Path) -> int:
...
def fetch_termux_contacts() -> list[dict[str, Any]]:
    proc = subprocess.run(
    ...
    *,
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        if not phone_col or not name_col:
            msg = f"CSV must have 'phone' and 'name' columns (got: {list(reader.fieldnames)})"
            raise ValueError(msg)
...
def resolution_stats(conn: sqlite3.Connection) -> dict[str, int]:
