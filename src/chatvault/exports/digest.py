...
def _load_whisper_transcripts(
    conn: sqlite3.Connection, msg_ids: list[str], placeholders: str
) -> dict[str, dict[str, Any]]:
    """Return {message_id: {text, language, model}} for any transcribed audio."""
    if not msg_ids:
        return {}
    try:
        rows = conn.execute(
            f"SELECT message_id, text, language, model "
            f"FROM whisper_transcriptions WHERE message_id IN ({placeholders})",
            msg_ids,
        )
    except sqlite3.OperationalError:
        # Pre-v2 archive without the whisper_transcriptions table.
        return {}
...
def render_digest(
    ...
    *,
    ...
    chat_row = conn.execute("SELECT subject, kind FROM chats WHERE jid = ?", (chat_jid,)).fetchone()
    if not chat_row:
        msg = f"chat not found: {chat_jid}"
        raise LookupError(msg)
    resolver = NameResolver(conn)
    chat_title = chat_row["subject"] or (
        resolver.resolve(chat_jid) if chat_row["kind"] in ("user", "lid") else chat_jid
    )
    ...
    for offset, m in enumerate(msgs):
        ...
        if tr := transcriptions.get(m["id"]):
            tag = f"transcript ({tr['language'] or 'auto'})"
            out.append(f"  _[{tag}]_  ")
        ...
        if rxs := reactions_by_msg.get(m["id"]):
            ...
            parts = [f"{e}×{len(ns)} ({', '.join(sorted(set(ns)))})" for e, ns in by_emoji.items()]
