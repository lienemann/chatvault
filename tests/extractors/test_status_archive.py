...
def test_extract_own_writes_only_public_statuses(populated_source, archive_db) -> None:
    ...
    rows = list(archive_db.execute("SELECT id, kind, chat_jid, text FROM status_archive"))
    assert len(rows) == 1
...
def test_extract_received_writes_status_with_media_and_thumb(archive_db, tmp_path) -> None:
    ...
    sb.execute(
        "INSERT INTO status_media_link(row_id, status_row_id, media_content_row_id) VALUES(1, 1, 1)"
    )
    sb.execute(
