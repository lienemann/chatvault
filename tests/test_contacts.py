...
def test_sync_contacts_round_trip(archive_db, monkeypatch) -> None:
    payload = json.dumps(
        [
            {"name": "Alice", "number": "+41 79 123 45 67"},
            {"name": "Bob", "number": "0792224444"},
    ...
    assert rows == {
        "41791234567@s.whatsapp.net": "Alice",
        "41792224444@s.whatsapp.net": "Bob",
    }
...
def test_sync_contacts_idempotent(archive_db, monkeypatch) -> None:
    payload = json.dumps([{"name": "Alice", "number": "+41 79 123 45 67"}])
    monkeypatch.setattr("sys.stdin", io.StringIO(payload))
...
def test_sync_contacts_detects_removals(archive_db, monkeypatch) -> None:
    ...
    second = json.dumps([{"name": "Alice", "number": "+41 79 123 45 67"}])
...
def test_import_pins_csv_rejects_missing_columns(archive_db, tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match=r"phone.*name"):
...
def test_unresolved_senders_excludes_known(archive_db) -> None:
    ...
    assert "41791234567@s.whatsapp.net" not in phones  # Alice already known
