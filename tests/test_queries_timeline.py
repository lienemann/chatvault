...
def _seed(archive_db) -> None:
    archive_db.execute(
        ...
        "VALUES('4111@s.whatsapp.net', 'Alice', 'address_book', '2026-01-01T00:00:00Z')"
    )
    archive_db.execute(
        "INSERT INTO contacts(phone_jid, name, source, updated_at) "
        "VALUES('4222@s.whatsapp.net', 'Alice', 'address_book', '2026-01-01T00:00:00Z')"
    ...
    archive_db.execute("INSERT INTO chats(jid, kind, subject) VALUES('CG@g.us', 'group', 'Café')")
...
def test_timeline_expands_jids_via_contacts_and_identity_links(archive_db) -> None:
    _seed(archive_db)
    rows = timeline_for_member(archive_db, "Alice", limit=10)
    assert len(rows) == 3
    # Most recent first
...
def test_timeline_returns_empty_for_unknown_name(archive_db) -> None:
    _seed(archive_db)
    assert timeline_for_member(archive_db, "Carol") == []
...
def test_forgotten_contacts_returns_no_outgoing(archive_db) -> None:
    ...
    assert names == {"Alice"}  # de-duplicated by chat_jid not by name; both are Annas
...
def test_resolve_chat_finds_person_by_contact_name(archive_db) -> None:
    ...
    archive_db.execute(
        "INSERT INTO contacts(phone_jid, name, source, updated_at) "
        "VALUES('41790001122@s.whatsapp.net', 'John Doe', 'address_book', "
        "       '2026-01-01T00:00:00Z')"
    )
    archive_db.execute(
        "INSERT INTO chats(jid, kind, subject) VALUES('41790001122@s.whatsapp.net', 'user', NULL)"
    )
    archive_db.execute(
        "INSERT INTO messages(id, source_rowid, chat_jid, sender_jid, from_me, ts, "
        "                     type, type_raw, key_id) "
        "VALUES('m1', 1, '41790001122@s.whatsapp.net', '41790001122@s.whatsapp.net', "
        "       0, '2026-04-01', 'text', 0, 'k1')"
    )
    archive_db.commit()
    assert resolve_chat(archive_db, "John") == "41790001122@s.whatsapp.net"
    assert resolve_chat(archive_db, "John Doe") == "41790001122@s.whatsapp.net"
...
def test_resolve_chat_finds_person_by_phone_number(archive_db) -> None:
    ...
    archive_db.commit()
    assert resolve_chat(archive_db, "+41 79 000 11 22") == "41790001122@s.whatsapp.net"
...
def test_list_chats_backfills_display_name_for_1on1(archive_db) -> None:
    ...
    assert by_jid["41790001122@s.whatsapp.net"]["display_name"] == "John Doe"
...
def test_members_for_returns_peer_and_owner_for_1on1(archive_db) -> None:
    ...
    archive_db.commit()
    members = members_for(archive_db, "41790001122@s.whatsapp.net")
    ...
    assert any(m["jid"] == "41790001122@s.whatsapp.net" for m in members)


def test_chat_message_at_python_indexing(archive_db) -> None:
    from chatvault.queries.chats import chat_message_at

    archive_db.execute("INSERT INTO chats(jid, kind, subject) VALUES('C@g.us', 'group', 'C')")
    archive_db.executemany(
        "INSERT INTO messages(id, source_rowid, chat_jid, sender_jid, from_me, ts, "
        "                     type, type_raw, key_id) "
...
def test_resolve_message_full_id_passthrough(archive_db) -> None:
    ...
    full = "41790001122@s.whatsapp.net:0:ABCDEF1234567890"
...
def test_resolve_message_chat_colon_int(archive_db) -> None:
    ...
    archive_db.execute(
        "INSERT INTO contacts(phone_jid, name, source, updated_at) "
        "VALUES('41791234567@s.whatsapp.net', 'Alice', 'manual', '2026-01-01T00:00:00Z')"
    )
    archive_db.execute(
        "INSERT INTO chats(jid, kind, subject) VALUES('41791234567@s.whatsapp.net', 'user', NULL)"
    )
    archive_db.executemany(
        "INSERT INTO messages(id, source_rowid, chat_jid, sender_jid, from_me, ts, "
        "                     type, type_raw, key_id) "
    ...
    assert resolve_message(archive_db, "Alice") == "41791234567@s.whatsapp.net:0:K3"
    # Chat:int positive
    assert resolve_message(archive_db, "Alice:1") == "41791234567@s.whatsapp.net:0:K1"
    # Chat:int negative (Python-style)
    assert resolve_message(archive_db, "Alice:-1") == "41791234567@s.whatsapp.net:0:K3"
    assert resolve_message(archive_db, "Alice:-2") == "41791234567@s.whatsapp.net:0:K2"
...
def test_resolve_chat_via_alias(archive_db, tmp_path, monkeypatch) -> None:
    ...
    save_chat_aliases(cfg, {"andi": "41790001122@s.whatsapp.net"})
    assert resolve_chat(archive_db, "andi") == "41790001122@s.whatsapp.net"
...
def test_resolve_chat_long_int_is_phone_not_cache_index(archive_db, tmp_path, monkeypatch) -> None:
    ...
    assert resolve_chat(archive_db, "41790001122") == "41790001122@s.whatsapp.net"
