...
def test_resolver_falls_back_to_phone(archive_db) -> None:
    archive_db.execute(
        ...
        "VALUES('41791234567@s.whatsapp.net', 'Alice', 'address_book', '2026-01-01T00:00:00Z')"
    )
    archive_db.execute(
    ...
    assert r.resolve("41791234567@s.whatsapp.net") == "Alice"
    assert r.resolve("100@lid") == "Alice"
...
def test_resolver_uses_real_display_name_for_lid(archive_db) -> None:
    archive_db.execute(
        ...
        "VALUES('300@lid', 'Bob', '2026-01-01T00:00:00Z')"
    )
    archive_db.commit()
    r = NameResolver(archive_db)
    assert r.resolve("300@lid") == "Bob"
