...
chatvault chat digest "Birthday Alice" --last 1000 -o birthday.md

# 1:1 chat by contact name
chatvault chat digest "John Doe" --last 200 -o john.md

# 1:1 by phone number
chatvault chat digest "+41 79 000 11 22" --last 200 -o john.md
...
```sh
chatvault chat export "John Doe" --out ./john
# ./john/digest.md          (markdown digest, all messages by default)
# ./john/digest.jsonl       (jsonl digest)
# ./john/media/             (hardlinked media files)
```
...
Format per message:
  ...
  **[ts] sender** _(type)_
    ...
    reactions: 👍×3 (Alice, Bob, Carol) (optional)
...
```sh
cat ~/.local/share/chatvault/prompts/digest.md birthday-alice.md | pbcopy
```
