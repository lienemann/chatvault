...
| Form | Example |
|---|---|
| Group subject substring | `"Acme Group"`, `"Daygame)"` |
| Contact name substring | `"John"`, `"John Doe"` (resolves to that person's 1:1 chat) |
| Phone number (any format) | `"+41 79 000 11 22"`, `"0041790001122"` |
| Literal JID | `41790001122@s.whatsapp.net`, `120363000@g.us` |
...
| Form | Example |
|---|---|
| Full message id | `41790001122@s.whatsapp.net:0:ACDD7C38…` |
| `<chat-ref>:<int>` | `John:1` (oldest in chat), `John:-1` (newest), `John:-2` (next-to-newest); chat-ref accepts every form above, including `3:-1` (newest message in chat #3) |
...
```sh
chatvault chat alias mom "Sam Roe"     # set
...
```sh
chatvault chat members "Acme Group"
chatvault chat members "Acme Group" --history    # include left members
chatvault chat members "John Doe"              # 1:1 → owner + peer
```
...
```sh
chatvault chat why "Acme Group" "196323038986257@lid"
```
...
```sh
chatvault timeline "Alice" --since 2026-01-01
```
...
chatvault search "rezept" --chat "Finance" --since 2026-01-01
```
